...
#define LSM6DSM_DATAWRITE_SLV0            0x0E


#define LSM6DSM_ADDRESS           0x6A   // Address of LSM6DSM accel/gyro when ADO = 0
#define LSM6DSM1                  0x6B
#define LSM6DSM2                  0x6A
...
class LSM6DSM
{
  ...
  uint8_t getChipID(uint8_t address);
  void passthruMode(uint8_t address);
  void masterMode(uint8_t address);
  void sleepMode(uint8_t address);
  void wakeMode(uint8_t address, uint8_t AODR, uint8_t GODR);
  void LIS2MDLSleepMode(uint8_t address, uint8_t MODR);
  void LIS2MDLWakeMode(uint8_t address, uint8_t MODR);
  void init(uint8_t address, uint8_t Ascale, uint8_t Gscale, uint8_t AODR, uint8_t GODR);
  void AGoffsetBias(uint8_t address, float * dest1, float * dest2);
  void MagoffsetBias(uint8_t address, float * dest1, float * dest2);
  void reset(uint8_t address);
  void selfTest(uint8_t address);
  void readAccelGyroData(uint8_t address, int16_t * destination);
  void readMagData(uint8_t address, int16_t * destination);
  int32_t readBaroData(uint8_t address);
  int16_t readBaroTemp(uint8_t address);
  private:
  float _aRes, _gRes;
  I2Cdev* _i2c_bus;
};
