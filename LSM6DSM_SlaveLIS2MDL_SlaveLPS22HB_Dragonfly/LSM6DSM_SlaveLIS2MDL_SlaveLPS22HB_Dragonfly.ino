...
LSM6DSM LSM6DSM(LSM6DSM_intPin1, &i2c_0); // instantiate LSM6DSM class
...
void setup() {
  ...
  pinMode(LSM6DSM_intPin1, INPUT); // enable LSM6DSM interrupt
  ...
  // Put LSM6DSM in master mode
  LSM6DSM.passthruMode(); // enable master mode to initialize all the sensors
  Serial.println("LSM6DSM in passthrough mode for sensor configuration!");
  Serial.println(" ");
  delay(1000); 
  ...
  if(c == 0x6A && d == 0x40 && e == 0xB1) // check if all I2C sensors have acknowledged
  {
   ...
   LSM6DSM.init(Ascale, Gscale, AODR, GODR); // configure LSM6DSM  

   LSM6DSM.selfTest();

   // Put LSM6DSM in master mode
   LSM6DSM.masterMode(); // enable master mode to initialize all the sensors
   Serial.println("LSM6DSM in master mode for sensor data acquisition!");
   Serial.println(" ");
   delay(100);

//   LSM6DSM.MagoffsetBias(magBias, magScale);
   ...
   LSM6DSM.AGoffsetBias(gyroBias, accelBias);
...
void loop() {
   ...
   if(newLSM6DSMData == true) {   // On interrupt, read data
     ...
     LSM6DSM.readAccelGyroData(LSM6DSMData); // INT1 cleared on any read
   ...
   // Calculate the gyro value into actual degrees per second
     ...
     LSM6DSM.readMagData(LIS2MDLData);
   ...
   if(SerialDebug)
    ...
    Pressure = (float)LSM6DSM.readBaroData()/4096.0f;
    Temperature = (float)LSM6DSM.readBaroTemp()/100.0f; 
